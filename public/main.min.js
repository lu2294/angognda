    ...
    L.EchartsLayer = L.Class.extend({
        ...
        _init: function (map, ec) {
            ...
            self.pixel2GeoCoord = function (pixel) {
                var point = self._map.containerPointToLatLng(
                        ...
                        if (data && data.length) {
                            for (var k = 0, len = data.length; k < len; k++) {
                                if (
                                    !(
                                        data[k].name &&
                                        this._geoCoord.hasOwnProperty(
                                            data[k].name
                                        )
                                    )
                                ) {
                                    data[k].name = k + 'markp'
                                    self._geoCoord[data[k].name] =
                                        data[k].geoCoord
                                }
                                self._AddPos(data[k])
                            }
                    ...
                    if (data && data.length) {
                        for (var k = 0, len = data.length; k < len; k++) {
                            if (
                                !(
                                    data[k].name &&
                                    this._geoCoord.hasOwnProperty(data[k].name)
                                )
                            ) {
                                data[k].name = k + 'markp'
            ...
            self._bindEvent = function () {
                ...
                self._ec.getZrender().on('mousewheel', function (e) {
                    ...
                    self._lastMousePos = self._map.mouseEventToContainerPoint(
                        e.event
                    )
            ...
            function _moveHandler(type) {
                return function () {
                    ...
                    // 记录偏移量
                    self._mapOffset = [
                        -parseInt(domPosition.x) || 0,
                        -parseInt(domPosition.y) || 0,
                    ]
                    self._echartsContainer.style.left =
                        self._mapOffset[0] + 'px'
